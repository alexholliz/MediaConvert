...
for file in */*.{mp4,MP4}; do
	...
	if [ "$VTRANSCODE" == 1 ] && [ "$ATRANSCODE" == 1 ]; then
		...
		mv "$file" "${file%.*}.bak"
		ffmpeg -y -i "${file%.*}.bak" -c:v libx264 -preset superfast -crf 23 -tune film -b:v 8M -maxrate:v 8M -bufsize:v 8M -c:a aac -strict experimental -ac 2 -b:a 256k "${file%.*}.mp4" && rm "${file%.*}.bak"
		...
		mv "$file" "${file%.*}.bak"
		ffmpeg -y -i "${file%.*}.bak" -c:v copy -c:a aac -strict experimental -ac 2 -b:a 256k "${file%.*}.mp4" && rm "${file%.*}.bak"
	elif [ "$VTRANSCODE" == 1 ] && [ "$ATRANSCODE" == 0 ]; then
		mv "$file" "${file%.*}.bak"
		ffmpeg -y -i "${file%.*}.bak" -c:v libx264 -preset superfast -crf 23 -tune film -b:v 8M -maxrate:v 8M -bufsize:v 8M -c:a copy "${file%}.mp4" && rm "${file%.*}.bak"
